...
void do_qtrust(CHAR_DATA *ch, char *argument)
{
    ...
    {
        if (can_give) act("$n tried to grant you qtrust, but failed because:",ch,NULL,victim,TO_VICT);
    ...
    for (obj = object_list; obj != NULL; obj = obj->next)
    {
        ...
        if (obj->carried_by != victim) continue;
        if (obj->questowner == NULL) continue;
        if (strlen(obj->questowner) < 2) continue;
