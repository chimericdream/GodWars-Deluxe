...
void multi_hit(CHAR_DATA *ch, CHAR_DATA *victim, int dt) {
    ...
    if (
        ...
        && IS_WEREWOLF(ch)
        && wieldorig == 0
        && get_tribe(ch, TRIBE_BLACK_FURIES) > 1
    ) {
...
int get_speed(CHAR_DATA *ch) {
    ...
    if (IS_STANCE(ch, STANCE_VIPER) && number_percent() > ch->stance[STANCE_VIPER] * 0.5) {
        ...
        if (number_percent() > (ch->stance[stance] * 0.5) && IS_SET(ch->pcdata->qstats[stance-11], QS_SUPER_SPEED)) {
            count--;
        }
    }
...
void one_hit(CHAR_DATA *ch, CHAR_DATA *victim, int dt, int handtype) {
    ...
    if (dt == TYPE_UNDEFINED) {
        ...
        if (wield != NULL && wield->item_type == ITEM_WEAPON) {
            dt += wield->value[3];
            if (
                (right_hand && IS_ITEMAFF(ch, ITEMA_RIGHT_AGG))
                ...
                if (
                    (right_hand && IS_ITEMAFF(ch, ITEMA_RIGHT_SILVER))
    ...
    if (IS_NPC(ch)) {
        ...
        if (IS_POLYAFF(ch, POLY_SERPENT)) {
            ...
            else if (!IS_NPC(ch) && IS_VAMPAFF(ch,VAM_CLAWS) && get_auspice(ch, AUSPICE_AHROUN) > 0) {
                dam = number_range(25, 50);
                dt += TYPE_ADD_AGG;
            }
    ...
    if (victim != ch) {
        ...
        if (dt >= TYPE_HIT) {
            ...
            */
            ...
            else if (!IS_NPC(victim) && victim->stance[0] >= 11 && victim->stance[0] <= 15) {
                int stance = victim->stance[0];

                if (IS_SET(victim->pcdata->qstats[stance-11], QS_SUPER_PARRY)) {
                    if (
                        victim->stance[stance] > 100
                        && !can_counter(ch)
                        && !can_bypass(ch,victim)
                        && check_parry(ch, victim, dt)
                    ) {
